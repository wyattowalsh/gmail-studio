function sendSingleEmail_(payload) {
  return deliverSingleEmail_(payload, 'SEND');
}

function createSingleDraft_(payload) {
  ...
  return withQueueLock_(function () {
    ...
    payloads.forEach(function (payload) {
      try {
        ...
        const result = deliveryMode === 'DRAFT' ? createSingleDraft_(payload) : sendSingleEmail_(payload);
        results.push(result);
...
function sendTestComposeDraft_() {
  const payload = getComposePayload();
  payload.recipient = 'wyattowalsh@gmail.com';
  return sendSingleEmail_(payload);
}

function sendComposeDraft_() {
  const payload = getComposePayload();
  return sendSingleEmail_(payload);
}

function createComposeDraft_() {
  const payload = getComposePayload();
  return createSingleDraft_(payload);
}

function sendSelectedOutboundRow_() {
  return withQueueLock_(function () {
    const payload = getSelectedOutboundPayload();
    const result = sendSingleEmail_(payload);
    updateOutboundRows([
      {
        rowNumber: payload.__rowNumber,
        patch: {
...
function createSelectedOutboundDraft_() {
  return withQueueLock_(function () {
    const payload = getSelectedOutboundPayload();
    const result = createSingleDraft_(payload);
    updateOutboundRows([
      {
        rowNumber: payload.__rowNumber,
        patch: {
...
function createScheduledDraftBatch_() {
...
if (typeof module !== 'undefined') {
  module.exports = {
    ...
    createComposeDraft: createComposeDraft_,
    createScheduledDraftBatch: createScheduledDraftBatch_,
    createSelectedOutboundDraft: createSelectedOutboundDraft_,
    createSingleDraft: createSingleDraft_,
    deliverSingleEmail_,
    getQuotaForecast: getQuotaForecast_,
    getBatchSettings_,
    isTrackingEnabled_,
    sendSingleEmail: sendSingleEmail_,
    sendTestComposeDraft: sendTestComposeDraft_,
    sendComposeDraft: sendComposeDraft_,
    sendSelectedOutboundRow: sendSelectedOutboundRow_,
    sendUnsentBatch: sendUnsentBatch_,
    sendScheduledBatch: sendScheduledBatch_,
